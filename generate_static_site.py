    ...
    <main class="main-container">
        ...
        <div id="tab3" class="tab-content">
            ...
            <div class="chart-box">
                ...
                <details class="st-expander" style="margin-top:14px;">
                    <summary>📄 ดูตาราง Matrix รายละเอียด</summary>
                    <div class="expander-content">
                        <table class="data-table">
                            <thead>
                                <tr>
                                    <th>ฝ่าย</th>
                                    <th style="text-align:center;">รอบที่ 1</th>
                                    <th style="text-align:center;">รอบที่ 2</th>
                                    <th style="text-align:center;">รอบที่ 3</th>
                                    <th style="text-align:center;">รอบที่ 4</th>
                                    <th style="text-align:right;" id="matrixTotalColHeader">รวม 4 รอบ</th>
                                </tr>
                            </thead>
                            <tbody id="tableMatrixBody"></tbody>
                        </table>
                    </div>
                </details>
            </div>
...
{data_json_str}
    ...
    <script>
        ...
        function renderHeatmap() {{
            ...
            // Compute statistics for each department across rounds 1-4
            const deptStats = depts.map(d => {{
                const r1_apps = RAW_DATA.choices.filter(c => c.department === d && c.round === 1).length;
                const r2_apps = RAW_DATA.choices.filter(c => c.department === d && c.round === 2).length;
                const r3_apps = RAW_DATA.choices.filter(c => c.department === d && c.round === 3).length;
                const r4_apps = RAW_DATA.choices.filter(c => c.department === d && c.round === 4).length;
                const total_apps = r1_apps + r2_apps + r3_apps + r4_apps;

                const r1_pass = RAW_DATA.choices.filter(c => c.department === d && c.round === 1 && c[passCol]).length;
                const r2_pass = RAW_DATA.choices.filter(c => c.department === d && c.round === 2 && c[passCol]).length;
                const r3_pass = RAW_DATA.choices.filter(c => c.department === d && c.round === 3 && c[passCol]).length;
                const r4_pass = RAW_DATA.choices.filter(c => c.department === d && c.round === 4 && c[passCol]).length;
                const total_pass = r1_pass + r2_pass + r3_pass + r4_pass;

                const r1_rate = r1_apps > 0 ? parseFloat(((r1_pass / r1_apps) * 100).toFixed(1)) : 0;
                const r2_rate = r2_apps > 0 ? parseFloat(((r2_pass / r2_apps) * 100).toFixed(1)) : 0;
                const r3_rate = r3_apps > 0 ? parseFloat(((r3_pass / r3_apps) * 100).toFixed(1)) : 0;
                const r4_rate = r4_apps > 0 ? parseFloat(((r4_pass / r4_apps) * 100).toFixed(1)) : 0;
                const total_rate = total_apps > 0 ? parseFloat(((total_pass / total_apps) * 100).toFixed(1)) : 0;

                return {{
                    dept: d,
                    apps: [r1_apps, r2_apps, r3_apps, r4_apps],
                    passed: [r1_pass, r2_pass, r3_pass, r4_pass],
                    rate: [r1_rate, r2_rate, r3_rate, r4_rate],
            ...
            if (metric === 'applications') {{
                deptStats.sort((a, b) => b.total_apps - a.total_apps);
            }} else if (metric === 'passed') {{
                deptStats.sort((a, b) => b.total_pass - a.total_pass);
            }} else {{
                deptStats.sort((a, b) => b.total_rate - a.total_rate);
            }}

            // Reverse for Plotly y-axis so top department is at top
            const displayDepts = [...deptStats].reverse();
            const yLabels = displayDepts.map(d => d.dept);

            const matrixZ = displayDepts.map(d => {{
                if (metric === 'applications') return d.apps;
                if (metric === 'passed') return d.passed;
                return d.rate;
            }});

            const textMatrix = displayDepts.map(d => {{
                if (metric === 'applications') return d.apps.map(v => String(v));
                if (metric === 'passed') return d.passed.map(v => String(v));
                return d.rate.map(v => v > 0 ? v + '%' : '0%');
            }});

            // Soft Light Pastel Colorscales (for maximum text legibility & clean white aesthetic)
            let colorscale;
            if (metric === 'applications') {{
                colorscale = [
                    [0, '#F8FAFC'],
            ...
            // Clean bold dark text on all soft pastel cells (100% crystal clear legibility)
            const annotations = [];
            for (let i = 0; i < displayDepts.length; i++) {{
                for (let j = 0; j < rounds.length; j++) {{
                    const textVal = textMatrix[i][j];
                    annotations.push({{
                        x: rounds[j],
                        y: yLabels[i],
                        text: `<b>${{textVal}}</b>`,
            ...
            Plotly.react('chartHeatmap', [{{
                ...
                colorbar: {{
                    ...
                    thickness: 12,
                    outlinewidth: 0,
                    tickfont: {{ family: 'Sarabun', size: 10 }}
                }},
                hoverongaps: false
            }}], {{
            ...
            // Populate Matrix Table in Expander
            const tableBody = document.getElementById('tableMatrixBody');
            const totalColHeader = document.getElementById('matrixTotalColHeader');
            if (tableBody && totalColHeader) {{
                tableBody.innerHTML = '';
                if (metric === 'applications') {{
                    totalColHeader.textContent = 'รวม 4 รอบ';
                }} else if (metric === 'passed') {{
                    totalColHeader.textContent = 'รวมผ่าน 4 รอบ';
                }} else {{
                    totalColHeader.textContent = 'อัตราผ่านรวม (%)';
                }}

                deptStats.forEach(d => {{
                    let r1, r2, r3, r4, tot;
                    if (metric === 'applications') {{
                        r1 = d.apps[0]; r2 = d.apps[1]; r3 = d.apps[2]; r4 = d.apps[3]; tot = d.total_apps;
                    }} else if (metric === 'passed') {{
                        r1 = d.passed[0]; r2 = d.passed[1]; r3 = d.passed[2]; r4 = d.passed[3]; tot = d.total_pass;
                    }} else {{
                        r1 = d.rate[0] + '%'; r2 = d.rate[1] + '%'; r3 = d.rate[2] + '%'; r4 = d.rate[3] + '%'; tot = d.total_rate + '%';
                    }}

                    tableBody.innerHTML += `
                        <tr>
                            <td style="font-weight:700;">${{d.dept}}</td>
                            <td style="text-align:center;">${{r1}}</td>
                            <td style="text-align:center;">${{r2}}</td>
                            <td style="text-align:center;">${{r3}}</td>
                            <td style="text-align:center;">${{r4}}</td>
                            <td style="text-align:right; font-weight:700; color:#E03177;">${{tot}}</td>
                        </tr>
                    `;
                }});
            }}

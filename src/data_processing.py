...
def normalize_department(dept_text) -> str:
    ...
    if not s:
        return ""
    # Strip leading numbers if like '06 - นิสิตสัมพันธ์'
    s = re.sub(r"^\d+\s*[-–.]\s*", "", s)
    dept_map = {
        "1": "นายกสโมสรนิสิต",
        "2": "อุปนายกคนที่ 1",
        ...
        "5": "เหรัญญิก",
        "6": "นิสิตสัมพันธ์",
        "7": "ฝ่ายศิลปะและวัฒนธรรม",
        "8": "ฝ่ายวิชาการ",
        "9": "กีฬา",
        "10": "พัฒนาสังคมและบำเพ็ญประโยชน์",
    }
    s_clean = s.replace(".0", "")
    if s_clean in dept_map:
        return dept_map[s_clean]
    if s in ["สิตสัมพันธ์", "นินิสิตสัมพันธ์"]:
        return "นิสิตสัมพันธ์"
    if s in ["วิชาการ", "ฝ่ายวิชาการ", "ฝ่ายฝ่ายวิชาการ"]:
        return "ฝ่ายวิชาการ"
    if s in ["กีฬา", "ฝ่ายกีฬา"]:
        return "กีฬา"
    if s in ["พัฒนาสังคมและบําเพ็ญประโยชน์", "พัฒนาสังคมและบำเพ็ญประโยชน์"]:
        return "พัฒนาสังคมและบำเพ็ญประโยชน์"
    return s
